...
export const editUser = (_id, firstname, lastname, roll, email, sadmin) => (dispatch) => {

  console.log(_id, firstname, lastname, roll, email, "ADMIN SENT.................")
  const newUser = {
    firstname: firstname,
    lastname: lastname,
    roll: roll,
  ...
  return fetch(baseUrl + 'users/' + _id, {
    ...
    .then(response => response.json())
    .then(response => {
      console.log(response._id, "IIIIIIIIIIIIII")
      var res = JSON.parse(localStorage.getItem('userinfo'));
      console.log(res, "RESPONSE................")
      if (res._id != _id) {
        // var res = localStorage.getItem('userinfo')
        console.log(JSON.stringify(res), "FFFFFFFFFFFFFFFFF");
        return dispatch(editUserdispatch(res));
      }
      else {
        localStorage.removeItem('userinfo');
        localStorage.setItem('userinfo', JSON.stringify(response));
        console.log(JSON.stringify(response), "RRRRRRRRRRRRRR");
...
export const postIssue = (bookId, studentId) => (dispatch) => {
  console.log(bookId, studentId, "FFFFFFFFFFFFFFFFFFFFF");
  const newIssue = {
    book: bookId,
    student: studentId
  };
...
export const statusIssue = (issueId, status) => (dispatch) => {
  const bearer = 'Bearer ' + localStorage.getItem('token');
  return fetch(baseUrl + 'issues/' + issueId, {
    method: "POST"
    , headers: {
    ...
    .catch(error => {
      alert('The book could not be accepted or rejected\nError: ' + error.message);
    });
};
