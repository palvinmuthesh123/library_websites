...
class Issue extends Component {
    ...
    componentDidMount() {
        window.scrollTo(0, 0)
        console.log(this.props,"PPPPPPPPPPPPPPPPPPPPP");
    }

    render() {
        if (this.props.auth.userinfo.admin || this.props.auth.userinfo.staff )
        {
        if (this.props.booksLoading || this.props.usersLoading) {
            ...
            let useroptions = this.props.users.filter((user) => (!user.admin && !user.staff));
    ...
    else
    {
        if (this.props.booksLoading) {
            ...
            const defaultBook = this.props.books[0];
            if (this.state.isbn === '') {
                this.setState({ isbn: defaultBook.isbn, roll: this.props.auth.userinfo.roll });
            }
            return (
                <div className="container full">
                    <div className="row justify-content-center heading">
                    ...
                    <div className="row row-content justify-content-center">
                        <Form onSubmit={(e) => {
                            let bookid = this.props.books.filter((book) => (book.isbn === this.state.isbn))[0]._id
                            let studentid = this.props.auth.userinfo._id;
                            this.props.postIssue(bookid, studentid);
                            e.preventDefault();
                        }}>
                            ...
                            <FormGroup row>
                                ...
                                <Input type="select" 
                                    ...
                                    onInput={(e) => { this.setState({ isbn: e.target.value }) }}>
                                    {bookoptions}
                                </Input>
                            </FormGroup>
                            <FormGroup row>
                                ...
                                <Input 
                                    ...
                                    value={this.props.auth.userinfo.roll}
                                    className="form-control"
                                    // onInput={(e) => { this.setState({ roll: e.target.value }) }}
                                    disabled
                                >
                            ...
                            <FormGroup row>
                                ...
                                <Input 
                                    ...
                                    name="name_student"
                                    placeholder="Name of student"
                                    defaultValue={this.props.auth.userinfo.firstname + ' ' + this.props.auth.userinfo.lastname}
                                    // value={!this.state.roll ? ''
                                    //     : this.props.users.filter((user) => (user.roll === this.state.roll))[0].firstname + ' ' +
                                    //     this.props.users.filter((user) => (user.roll === this.state.roll))[0].lastname}
                                    className="form-control" disabled />
                            ...
                            <FormGroup row>
                                ...
                                <Input type="text" id="username" name="username"
                                    placeholder="Username of student"
                                    defaultValue={this.props.auth.userinfo.username}
                                    value={this.props.auth.userinfo.username}
                                    className="form-control" disabled />
                            </FormGroup>
                            <Row className="align-self-center">
                                <Col className="text-center">
